...
void Input::update()
{
	...
	windowCenter = Vec2((float)(clientRect.left + clientRect.right) / 2.0f, (float)(clientRect.top + clientRect.bottom) / 2.0f);
...
Math::Vec2 Input::getMousePos()
{
	POINT pnt;
	GetCursorPos(&pnt);
	return Vec2((float)pnt.x, (float)pnt.y);
}
...
void Input::updateMousePos(LPARAM lparam)
{
	...
	if (RIM_TYPEMOUSE == input.header.dwType)
	{
		...
		mousePos.x = (float)input.data.mouse.lLastX;
		mousePos.y = (float)input.data.mouse.lLastY;
	}
}
