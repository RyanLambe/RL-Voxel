...
int main()
{
    ...
    while (renderer.Render()) {
		...
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_I))
			rot.x -= 0.01f;
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_J))
			rot.y -= 0.01f;
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_K))
			rot.x += 0.01f;
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_L))
			rot.y += 0.01f;
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_O))
			rot.z += 0.01f;
		if (glfwGetKey(renderer.GetWindow(), GLFW_KEY_U))
			rot.z -= 0.01f;
		cam.Rotate(rot);
    }
